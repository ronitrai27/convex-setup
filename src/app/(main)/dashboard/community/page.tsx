...
import { Loader2, Rocket, Star, Users, ExternalLink, HeartPulse } from "lucide-react";
...
const CommunityPage = () => {
  ...
  useEffect(() => {
    const triggerAISearch = async () => {
      ...
      setAiLoading(true);
      try {
        const res = await fetch(`/api/ai/search?query=${encodeURIComponent(query)}&mode=${mode}`);
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8 space-y-2">
        ...
        <p className="text-sm">
          Discover {mode === "team" ? "teams looking for help" : mode === "contribute" ? "open source opportunities" : "innovative projects"} ranked by health and activity.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 gap-6 max-w-xl mx-auto">
          {projects && projects.length > 0 ? (
              ...
              >
                <Card className="h-full flex flex-col overflow-hidden border-2 hover:border-primary/50 transition-colors bg-card/50 backdrop-blur-sm">
                  ...
                  <CardContent className="grow">
                    <div className="flex flex-wrap gap-2 mb-4">
                      {project.tags.slice(0, 3).map((tag: string) => (
                        <Badge key={tag} variant="outline" className="text-[10px] opacity-70 italic font-thin">
                          #{tag}
                        </Badge>
                      ))}
                    </div>
                    
                    {project.lookingForMembers && project.lookingForMembers.length > 0 && (
                      <div className="mt-4 p-3 rounded-lg bg-muted/30 border border-muted/50">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {project.lookingForMembers.map((m: any, idx: number) => (
                            <span key={idx} className="text-[10px] bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded-full capitalize font-thin italic">
                              {m.role}
                            </span>
                          ))}
                        </div>
                  ...
                  <CardFooter className="flex gap-2 pt-4 border-t bg-muted/10">
                    <Button variant="default" size="sm" className="flex-1 gap-2 group" onClick={() => window.open(project.repoUrl, '_blank')}>
                      View Repo
            ...
            <div className="col-span-full py-20 text-center space-y-4">
              ...
              <h3 className="text-xl font-semibold italic opacity-40">No projects found matching your search.</h3>
              <p className="text-muted-foreground italic font-thin">Try adjusting your query or filters.</p>
            </div>
          )}
        </div>
      </AnimatePresence>
